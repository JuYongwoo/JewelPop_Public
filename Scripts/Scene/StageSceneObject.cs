...
{
    public class StageSceneObject : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            levelManager.OnStart(GameManager.instance.resourceManager.levelDatasJSONHandle.Result.text); //현재 스테이지에 따라 맞는 레벨 데이터 로드(지금은 Level_1만)// dict 키값은 서버에서 받아오는 것으로
            mapManager.OnStart(levelManager.currentLevel);
